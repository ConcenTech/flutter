...
bool OnScreenKeyboardWin::EnsureInputPane(HWND hwnd) {
  ...
  // InputPane is apartment-affine to the platform STA that called
  // GetForWindow, so Showing/Hiding run on the platform thread. Apply
  // visibility updates synchronously; no task-runner hop is required.
